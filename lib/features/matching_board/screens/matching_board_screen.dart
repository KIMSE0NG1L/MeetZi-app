...
class _MatchingBoardScreenBodyState extends State<_MatchingBoardScreenBody> {
  ...
  Future<bool> _takeNote(String profileId, Map<String, dynamic> profile) async {
    ...
    if (matchingTicket <= 0) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('留ㅼ묶沅뚯씠 遺議깊빐?? ?곸젏?먯꽌 留ㅼ묶沅뚯쓣 援щℓ??二쇱꽭??')),
      );
      return false;
    }
    ...
    if (trimmed.length < 5) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('硫붿떆吏??5???댁긽 ?낅젰??二쇱꽭??')),
      );
      return false;
    }
    try {
      ...
      if (!mounted) return false;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('?붿껌??蹂대깉?댁슂. ?곷?媛 ?섎씫?섎㈃ 留ㅼ묶???깆궗?⑸땲??')),
      );
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            _isValid
                                ? '醫뗭븘?? ?댁젣 ?꾩넚?????덉뼱??'
                                : '理쒖냼 $_minLength???댁긽 ?낅젰??二쇱꽭??(?꾩옱 ${_controller.text.length}??',
                            style: TextStyle(
                              fontSize: 14,
                              color: _isValid
